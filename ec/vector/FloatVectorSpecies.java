...
public class FloatVectorSpecies extends VectorSpecies
    ...
    public final static String CHANGE_TO_CERO_MUT_PROB = "change-to-cero-mut-prob";
    public final static String CHANGE_RSU_PROB_MUT = "change-rsu-prob-mut";
    public final static String GAUSSIAN_NOICE_MUT_PROB = "gaussian-noice-mut-prob";

    public final static String SEGMENTS_PATH = "segments-path";
    public final static String RSU_TYPES_PATH = "rsu-types-path";
    public final static String GREEDY_RESULT_PATH = "greedys-res-path";

    public final static String COORDINATES_AMOUNT = "coordinates-amount";
    ...
    protected ArrayList<RsuType> rsu_types;
    protected float change_to_cero_mut_prob;
    protected float change_rsu_mut_prob;
    protected float gaussian_noice_mut_prob;
    protected int special_initializaion;
    protected float [][] greedy_results;

    public float [][] getGreedyResults(){
        return greedy_results;
    }

    public int getSpecialInitalization(){
        return special_initializaion;
    }

    public void decreaseSpecialInitalization(){
        special_initializaion--;
    }

    public float getChangeToCeroMutProb(){
        return change_to_cero_mut_prob;
    }

    public float getChangeRsuMutProb(){
        return change_rsu_mut_prob;
    }

    public float getGaussianNoiceMutProb(){
        return gaussian_noice_mut_prob;
    }
    ...
    public ArrayList<RsuType> getRsuTypes(){
        return rsu_types;
    }
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        // Load mutaction probabilities
        change_to_cero_mut_prob = state.parameters.getFloat(base.push(CHANGE_TO_CERO_MUT_PROB),def.push(CHANGE_TO_CERO_MUT_PROB), 0);
        change_rsu_mut_prob = state.parameters.getFloat(base.push(CHANGE_RSU_PROB_MUT),def.push(CHANGE_RSU_PROB_MUT), 0);
        gaussian_noice_mut_prob = state.parameters.getFloat(base.push(GAUSSIAN_NOICE_MUT_PROB),def.push(GAUSSIAN_NOICE_MUT_PROB), 0);

        segments = new ArrayList<Segment>();
        rsu_types = new ArrayList<RsuType>();

        try{
            // Load Segments
            String segments_path = state.parameters.getStringWithDefault(base.push(SEGMENTS_PATH), def.push(SEGMENTS_PATH), null);

            File fin = new File(segments_path);
            ...
            for (int i = 0; i < genomeSize; i++){
                ...
                segments.add(new Segment(start, end, importance));
            }
            br.close();


            // Load rsu types
            String rsu_types_path = state.parameters.getStringWithDefault(base.push(RSU_TYPES_PATH), def.push(RSU_TYPES_PATH), null);
            fin = new File(rsu_types_path);
            ...
            for (int i = 1; i < Math.round(maxGene[0]); i++){
                ...
                float cost = Float.parseFloat(line_tokens[2]);
                float radius = Float.parseFloat(line_tokens[3]);
                rsu_types.add(new RsuType(cost, radius));
            }
            br.close();

            // Load greedy results
            greedy_results = new float[18][];
            for (int i = 0; i < 18; i++) {
                greedy_results[i] = new float[genomeSize];
            }

            String greedys_path = state.parameters.getStringWithDefault(base.push(GREEDY_RESULT_PATH), def.push(GREEDY_RESULT_PATH), null);
            fin = new File(greedys_path);
            fis = new FileInputStream(fin);
            br = new BufferedReader(new InputStreamReader(fis));
            ...
            for (int i = 0; i < 18; i++){
                ...
                for (int j = 0; j < genomeSize; j++) {
                    greedy_results[i][j] = Float.parseFloat(line_tokens[j]);
                }
            }
            br.close();

    ...
    public void defaultMutate(EvolutionState state, int thread)
        {
            ...
            for(int i = 0; i < genome.length * 0.1; i++) {
    	        int random_position = random.nextInt(genome.length);

    	        double random_mutation = random.nextFloat();
    	        int tipo_infraestructura=(int) Math.floor(genome[random_position]);
    	        if (random_mutation < s.getChangeToCeroMutProb()){
    	        	// Remove rsu
    	        	genome[random_position] = genome[random_position] - tipo_infraestructura;
    	        }
    	        else if (random_mutation < s.getChangeToCeroMutProb() + s.getChangeRsuMutProb()){
    	        	// Change rsu type
    	        	genome[random_position] = genome[random_position] - tipo_infraestructura;
    	        	genome[random_position] = genome[random_position] + 1 + random.nextInt(3);
    	        }
    	        else{
    	        	// Gaussian mutation
    	        	gaussianMutation(state, random, s, random_position);
    	        }
            }
        }
...
//        double[] max_realvar = s.maxGenes;
    ...
    public void reset(EvolutionState state, int thread)
        {
        FloatVectorSpecies s = (FloatVectorSpecies) species;
        int ini_especial = s.getSpecialInitalization();

        if (ini_especial > 0){
            for (int x = 0; x < genome.length; x++) {
                genome[x] = s.getGreedyResults()[ini_especial - 1][x];
            }
            s.decreaseSpecialInitalization();
        }
            ...
            for (int x = 0; x < genome.length; x++)
                ...
                System.out.println(random.nextFloat() * (maxGene - minGene) + minGene);
                genome[x] = random.nextFloat() * (maxGene - minGene) + minGene;
                }
        }
        }
